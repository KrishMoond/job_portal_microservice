package com.jobportal.verification.outbox;
    ...
    public void processOne(OutboxEvent event) {
        try {
            ...
            if (retries >= MAX_RETRIES) {
                ...
                log.error("[OUTBOX] Dead-lettered eventId={} after {} attempts.", event.getId(), retries);
            } else {
                log.warn("[OUTBOX] Retry {}/{} for eventId={}: {}", retries, MAX_RETRIES, event.getId(), e.getMessage());
            }
            outboxEventRepository.save(event);
        }
    }
